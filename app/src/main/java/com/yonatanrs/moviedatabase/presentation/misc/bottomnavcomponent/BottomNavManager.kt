...
class BottomNavManager(
    ...
    private val firstBottomNavigationItemId: Int
) {
    companion object {
    ...
    private var navHistory = BottomNavHistory().apply {
        if (bottomNavigationView.menu.isEmpty()) {
            throw IllegalStateException("Bottom navigation view menu cannot be empty.")
        }
        push(firstBottomNavigationItemId)
    }
    ...
    private fun isFirstTab(): Boolean {
        ...
        return bottomNavigationView.selectedItemId == firstBottomNavigationItemId
    }
